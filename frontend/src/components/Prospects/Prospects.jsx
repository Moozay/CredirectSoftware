...
const Prospects = ({prospect, setProspect,isOpen,initialize, onClose, onOpen}) => {
  ...
  const handleClick = async (prospectData) => {
  ...
  const filteredList = useMemo(getFilteredList, [prospects, searchParams]);

  const handleDelete = async (event, prospect) => {
    event.preventDefault();
    const response = await axiosInstance.delete(
      `/prospects/${prospect.prospect_id}`
    );
  ...
  return (
    <>
      <Prospect isOpen={isOpen} onClose={onClose} prospect={prospect} />
                  ...
                  return (
                    <Tr key={prospect.prospect_id}>
                      ...
                      >
                        ...
                        <Link
                          to={"/dashboard/demandeCredit"}
                          onClick={() => createNewRecord(prospect)}
                        >
                          <IconButton
                            variant="outline"
                            color="blue.400"
